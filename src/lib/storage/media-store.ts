...
class BlobMediaStore implements MediaStore {
  ...
  async readCiphertext(assetId: string, expectedBytes: number): Promise<Uint8Array> {
    ...
    for (const part of parts) {
      ...
      if (!response.ok) throw new Error(`ciphertext part fetch failed: ${response.status}`);
  ...
  async readPreview(assetId: string): Promise<Uint8Array | null> {
    const blob = await head(previewKey(assetId)).catch(() => null);
    if (!blob) return null;
  ...
  private async listParts(assetId: string): Promise<{ offset: number; url: string }[]> {
    const parts: { offset: number; url: string }[] = [];
    let cursor: string | undefined;
    do {
      const page = await list({ prefix: `wtr/ciphertext/${assetId}/`, cursor });
      for (const blob of page.blobs) {
        const match = /\/([0-9a-f]{16})\.part$/.exec(blob.pathname);
        if (match) parts.push({ offset: parseInt(match[1], 16), url: blob.url });
      }
      cursor = page.cursor;
    } while (cursor);
